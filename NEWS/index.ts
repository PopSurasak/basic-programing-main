...
const covidDeath = database.filter(function(element, index) {
    ...
    return({
        province: element.province,//ใส่(,)ด้วย
        tota_death: element.total_death
    })
})
...
const newsthais = news.results.filter(function (element, index) {//filterเลือกได้แค่สองอัน คืออันที่จะเอากับไม่เอา
    return element.language !== 'english' && element.source_name === 'Thai Pbs News'//เขียนให้เหมือนต้นฉบับ
